window.RaceDB = (() => {
  const races = Object.freeze([
    ...
    { name: "Tefiling", aliases: ["tefiling"], hpBase: 14, pmBase: 0 },
    ...
    { name: "Prismares", aliases: ["prismares"], hpBase: 15, pmBase: 3 },
  ...
  function uniqueAliases(race) {
    const raw = [race.name, ...(race.aliases || [])];
  ...
  for (const race of races) {
    for (const alias of uniqueAliases(race)) {
      byAlias.set(alias, race);
    }
  }
  ...
  function findRace(input) {
    ...
    for (const [alias, race] of byAlias.entries()) {
      if (!startsWithMatch && alias.startsWith(key)) {
        startsWithMatch = race;
      }

      if (!includesMatch && (alias.includes(key) || key.includes(alias))) {
        includesMatch = race;
      }
    }
